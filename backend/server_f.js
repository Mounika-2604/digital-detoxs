// login-app/server.js - FRONTEND SERVER (Port 3000)
...
function evaluateEmergencyRequest(reason, appId) {
  ...
  if (score >= 13) {
    ...
    return {
      granted: false,
      score,
      message: `Request denied. Not enough justification.`
    };
  }
}
  ...
  try {
    ...
    for (let key in usageMap) {
      if (usageMap[key].minutes > mostUsed.minutes) {
        mostUsed = { app: key, minutes: usageMap[key].minutes };
      }
    }
    ...
    res.json({ 
      ...
      data: {
        ...
        history: todayUsage.slice(-6).reverse(),
...
app.post('/api/usage', requireAuth, async (req, res) => {
  try {
    const { appId, appName, minutes } = req.body;
    const user = await User.findOne({ email: req.session.user.email });
    ...
    if (!appId || !appName || !minutes || minutes <= 0) {
    ...
    if (!hasEmergency) {
      const currentUsage = getTodayUsage(user.usageData, appId);
      const limit = APP_LIMITS[appId];
      
      if (limit && (currentUsage + minutes) > limit.dailyLimit) {
        return res.status(403).json({ 
          success: false, 
          error: 'Daily limit exceeded',
          message: `This would exceed your daily limit for ${appName}`
        });
      }
    }
    ...
    res.json({ success: true, message: 'Usage logged', entry });
  } catch (error) {
...
app.get('/api/limits', requireAuth, async (req, res) => {
  try {
    ...
    const limitsWithUsage = Object.entries(APP_LIMITS).map(([appId, config]) => {
      const used = getTodayUsage(user.usageData, appId);
      return {
        appId,
        name: config.name,
        ...
        percentage: Math.min(100, (used / config.dailyLimit) * 100)
      };
    });
    
    res.json({ success: true, limits: limitsWithUsage });
  } catch (error) {
...
app.post('/api/focus', requireAuth, async (req, res) => {
  try {
    ...
    await user.save();
    res.json({ success: true, message: 'Focus session started' });
  } catch (error) {
...
app.post('/api/focus/end', requireAuth, async (req, res) => {
  try {
    ...
    if (!user || !user.activeFocusSession) {
      return res.status(400).json({ success: false, error: 'No active session' });
    }
    
    user.focusSessions.push(user.activeFocusSession);
    user.activeFocusSession = undefined;
    await user.save();
...
app.post('/api/emergency-access/request', requireAuth, async (req, res) => {
  try {
    ...
    if (!appId || !reason) {
      return res.status(400).json({ success: false, error: 'App ID and reason required' });
    }
...
console.log('✅ Frontend server loaded');